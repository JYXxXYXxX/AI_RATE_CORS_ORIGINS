...
class ProxyRuntime:
    ...
    def predict(
        ...
        *,
        ...
        vector = np.array([float(features.get(name, 0.0)) for name in artifact["feature_names"]], dtype=float)
        ...
        mae = float(artifact.get("metrics", {}).get("val_mae", artifact.get("metrics", {}).get("mae", 0.08)))
        margin = max(0.04, min(0.18, mae * 1.35))
        confidence = max(0.45, min(0.94, 0.88 - mae * 1.3 + min(0.08, artifact.get("train_count", 0) / 100)))
        return {
