...
def get_proxy_training_service(
    settings: Settings = Depends(get_settings),
) -> ProxyTrainingService:
...
def ensure_document_access(
    document_id: str, auth: AuthContext, repository: Any
) -> None:
    user_id = str(auth.user["id"]) if auth.user is not None else None
    if not repository.can_user_access_document(
...
def charge_analysis_credit(
    *, user_id: str, settings: Settings, source_type: str, source_id: str, note: str
) -> bool:
    try:
