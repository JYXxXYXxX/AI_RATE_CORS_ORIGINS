...
class CnkiCalibrator:
    ...
    def predict_range(
        self, ai_like_score: float, dispersion: float, subject: str | None = None
    ) -> CnkiRiskRange:
        center = self._predict_center(ai_like_score, subject)
        base_margin = (
            0.10 if len(self.samples) < self.settings.calibration_min_samples else 0.07
        )
    ...
    def confidence(self, segment_count: int, dispersion: float) -> float:
        sample_factor = min(
            0.22,
            len(self.samples) / max(self.settings.calibration_min_samples, 1) * 0.22,
        )
    ...
    def _predict_center(self, ai_like_score: float, subject: str | None) -> float:
        ...
        relevant = [
            sample for sample in self.samples if subject and sample.subject == subject
        ]
        ...
        numerator = sum(
            (sample.ai_like_score - x_mean) * (sample.cnki_ai_rate - y_mean)
        ...
        blended = 0.72 * (intercept + slope * ai_like_score) + 0.28 * _fallback_mapping(
            ai_like_score
        )
