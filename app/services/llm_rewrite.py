...
def _user_prompt(
    ...
    parts.append("=" * 40)
    parts.append(
        "改写边界：只允许围绕原句做贴身改写。不要补写“为回应前文需求”“本文将”“系统进一步”等原文没有的新铺垫；不要新增新模块、新业务、新场景；保留原句对象、事实和列举项。"
    )
    if section_title:
...
def _looks_like_local_rewrite(original: str, candidate: str) -> bool:
    ...
    if length_ratio < 0.6 or length_ratio > 1.35:
        return False
...
def _simple_rewrite_sentence(sentence: str, risk_type: str) -> str:
    ...
    if risk_type in ("aigc", "mixed"):
        ...
        rewritten = re.sub(
            ...
            "回顾上述发现，最值得关注的是实验中出现的异常数据和未达预期的结果",
            rewritten,
        )
        # 英文常见模板句
        rewritten = re.sub(
...
def _direct_rule_rewrite(sentence: str, risk_type: str) -> str:
    ...
    if cn_chars > en_chars:
        ...
        if re.search(r"随着[^，。；]+的发展", sentence):
            return re.sub(
                r"随着([^，。；]+)的发展",
                lambda m: f"近几年，{m.group(1)}在具体场景中应用增多",
                sentence,
            )
        return sentence
    ...
    if sentence.endswith("."):
        rewritten = sentence.rstrip(".") + ", which has been verified in related practice."
    else:
        rewritten = sentence + ", which has been verified in related practice."
    rewritten = re.sub(r"\bdesign and implementation\b", "development and deployment", rewritten, flags=re.IGNORECASE)
    rewritten = re.sub(r"\bbased on\b", "built upon", rewritten, flags=re.IGNORECASE)
    rewritten = re.sub(r"\bsystem\b", "platform", rewritten, flags=re.IGNORECASE, count=1)
