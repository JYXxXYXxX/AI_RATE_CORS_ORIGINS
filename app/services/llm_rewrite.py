...
from app.services.rewrite_strategy import (
    ...
    apply_learned_rewrite_style,
)
from app.services.feedback_learning import load_feedback_learning_guidance
...
## 输出要求
0. **最小改写原则**：你只能围绕用户原句做局部重写，不要额外补写背景、动机、前文、后文、系统价值、需求分析，也不要凭空新增原文没有出现的新场景、新模块、新结论。
...
3. **对每句给出**：
   ...
   - `rewritten`：【必须是可直接替换的完整改写句，严禁返回建议/指导/说明文字】将原句改写后的完整版本，语言必须和原句一致。用户应该能直接复制粘贴替换原文，不需要再自己修改。绝对不能返回类似"建议...""应该...""需要...""请...""拆开...替换...避免..."这样的指导性文字。
...
_STRICT_INLINE_REWRITE_RULES = """

## 最高优先级：逐字微改模式
用户要求的是类似 Word 修订中的“绿色文字逐字替换”，不是重新写一段文章。你必须遵守：
1. 只改原文中确实存在的词、短语、句式和连接词；不要新增原文没有的信息、模块、场景、结论、数据、时间、人物、研究动作。
2. 不要把一句话扩写成一整段；单句改写长度必须控制在原句的 70%-125% 之间。
3. 原句中的专有名词、系统名称、技术名词、章节对象、功能列表和事实顺序必须保留。
4. 优先做“小刀式修改”：删套话、换连接词、调整语序、替换少量高 AI 词、拆掉过度工整表达。
5. `rewritten_paragraph` 只能由原段落按 `sentences` 中的替换结果拼回去，不能另起炉灶重写。
6. 如果拿不准，就少改；宁可只替换 3-8 个词，也不要补充任何凭空内容。
7. `explanation` 只解释改了哪些词，不要要求用户再去补资料。
"""


def _system_prompt(risk_type: str, learning_store_path: str | None = None) -> str:
    dynamic_guidance = (
        load_feedback_learning_guidance(learning_store_path)
        if learning_store_path
        else ""
    )
    if risk_type == "aigc":
        return _SYSTEM_PROMPT_AIGC + _STRICT_INLINE_REWRITE_RULES + "\n\n" + LEARNED_REPORT_STRATEGY + dynamic_guidance
    if risk_type == "duplication":
        return _SYSTEM_PROMPT_DUPLICATION + _STRICT_INLINE_REWRITE_RULES + "\n\n" + LEARNED_REPORT_STRATEGY + dynamic_guidance
    return _SYSTEM_PROMPT_MIXED + _STRICT_INLINE_REWRITE_RULES + "\n\n" + LEARNED_REPORT_STRATEGY + dynamic_guidance
...
def _user_prompt(
    ...
    parts.append("=" * 40)
    parts.append(
        "改写边界：逐字微改，只能改原句里的词、短语、语序和少量连接词。不要补写“为回应前文需求”“本文将”“系统进一步”等原文没有的新铺垫；不要新增新模块、新业务、新场景、新结论、新数据；保留原句对象、事实、专有名词和列举项。"
    )
    parts.append(
        "输出约束：每条 rewritten 必须与 original 一一对应；长度控制在原句 70%-125%；rewritten_paragraph 只能把这些 rewritten 替换回原段落，不允许整段另写。"
    )
    if section_title:
    ...
    if cnki_dup_percent is not None or cnki_aigc_percent is not None:
        ...
        # 官方报告只作为校准标尺，不在用户可见建议里渲染“本地预测 vs 官方结果”的冲突。
        if local_aigc_score is not None and cnki_aigc_percent is not None:
            gap = cnki_aigc_percent - local_aigc_score * 100
            if gap > 15:
                parts.append(
                    f"【校准提示】官方报告的 AIGC 风险更高，改写优先级和力度按官方报告执行。"
                )
                parts.append(
                    "输出给用户时不要比较本地预测和官方结果，也不要声称系统漏检；只说明应按官方标记片段深度改写。"
                )
        if local_dup_score is not None and cnki_dup_percent is not None:
            gap = cnki_dup_percent - local_dup_score * 100
            if gap > 15:
                parts.append(
                    f"【校准提示】官方报告的查重风险更高，重复片段处理顺序和改写力度按官方报告执行。"
                )
                parts.append(
                    "输出给用户时不要比较本地预测和官方结果；只围绕官方标记的相似来源、风险等级和句式重构建议展开。"
                )
        if cnki_aigc_percent is not None and cnki_aigc_percent >= 30:
            parts.append(
                "注意：知网 AIGC 率偏高（≥30%），但仍然只能在原句范围内做词语、语序和句式微调；没有出现在原文中的研究细节、具体数据和主观判断一律不得新增。"
            )
        if cnki_dup_percent is not None and cnki_dup_percent >= 20:
            parts.append(
                "注意：知网查重率偏高（≥20%），但不能凭空扩写；只允许在原句事实范围内调整主语、语序、连接词和少量表达。"
            )
        parts.append("-" * 40)
    parts.append("=" * 40)
...
class LLMRewriteService:
    ...
    @classmethod
    def _parse_json_result(cls, content: str) -> dict[str, Any] | None:
        candidate = cls._strip_code_fences(content)
        if not candidate:
            return None
    ...
    def _normalize_rewrite_result(
        ...
        *,
        ...
        for item in raw_result.get("sentences") or []:
            if not isinstance(item, dict):
                continue
            original = str(item.get("original") or "").strip()
            rewritten = str(item.get("rewritten") or "").strip()
            if original and (not rewritten or rewritten == original or not _is_valid_rewritten(original, rewritten)):
                rewritten = _direct_rule_rewrite(original, risk_type)
        ...
        rebuilt_from_sentences = _rebuild_paragraph_from_sentence_rewrites(
            text, normalized_sentences, risk_type
        )
        rewritten_paragraph = str(raw_result.get("rewritten_paragraph") or "").strip()

        if normalized_sentences and _looks_like_local_rewrite(text, rebuilt_from_sentences):
            rewritten_paragraph = rebuilt_from_sentences

        if rewritten_paragraph and not _looks_like_local_rewrite(text, rewritten_paragraph):
            rewritten_paragraph = rebuilt_from_sentences

        if rewritten_paragraph and not _is_valid_rewritten(text, rewritten_paragraph):
            rewritten_paragraph = rebuilt_from_sentences if (normalized_sentences and _looks_like_local_rewrite(text, rebuilt_from_sentences)) else ""

        if not rewritten_paragraph or rewritten_paragraph.strip() == text.strip():
            rewritten_paragraph = fallback["rewritten_paragraph"]

        if not normalized_sentences:
            normalized_sentences = fallback["sentences"]

        return {
            "diagnosis": str(raw_result.get("diagnosis") or fallback["diagnosis"]).strip(),
            "sentences": normalized_sentences[:6],
            "rewritten_paragraph": rewritten_paragraph,
            "overall_advice": str(
                raw_result.get("overall_advice") or fallback["overall_advice"]
            ).strip(),
        }
    ...
    async def rewrite_paragraph(
        ...
        system_prompt = _system_prompt(
            risk_type, self.settings.feedback_learning_store_path
        )
        try:
            ...
            logger.warning(
                "Rewrite prompt rejected by injection guard, falling back to rule-based rewrite"
            )
            return _fallback_rewrite(
                text,
                risk_type,
        ...
        for attempt in range(max_retries):
            try:
                ...
                return self._normalize_rewrite_result(
                    raw_result=result,
                    text=text,
                    risk_type=risk_type,
                    reasons=reasons,
...
def _is_valid_rewritten(original: str, rewritten: str) -> bool:
    """检查 rewritten 是否是可直接替换的合格改写句，而不是建议/指导文字。"""
    if not rewritten or rewritten == original:
        return False

    # 禁止的建议性前缀/关键词
    banned_prefixes = (
        "建议", "应该", "需要", "请", "拆开", "替换", "避免", "补充", "添加",
        "修改", "删除", "检查", "注意", "这里", "尝试", "考虑", "尽量",
        "可", "可以", "应当", "须", "必须", "最好", "为了", "通过调整",
    )
    if any(rewritten.startswith(h) for h in banned_prefixes):
        return False
    # 如果 rewritten 包含明显的指导句式标记
    if any(h in rewritten for h in ("【请", "（请", "（建议", "（注意", "（需要")):
        return False

    original_compact = re.sub(r"\s+", "", original)
    rewritten_compact = re.sub(r"\s+", "", rewritten)
    if not original_compact or not rewritten_compact:
        return False
    length_ratio = len(rewritten_compact) / max(len(original_compact), 1)
    if length_ratio < 0.7 or length_ratio > 1.25:
        return False
    if _char_overlap_ratio(original_compact, rewritten_compact) < 0.65:
        return False

    # 语言一致性检查
    original_cn = sum(1 for ch in original if "\u4e00" <= ch <= "\u9fff")
    rewritten_cn = sum(1 for ch in rewritten if "\u4e00" <= ch <= "\u9fff")
    original_en = sum(1 for ch in original if ch.isascii() and ch.isalpha())
    rewritten_en = sum(1 for ch in rewritten if ch.isascii() and ch.isalpha())

    # 如果原文主要是中文，rewritten 也应该是中文
    if original_cn > original_en and rewritten_cn < rewritten_en:
        return False
    # 如果原文主要是英文，rewritten 也应该是英文
    if original_en > original_cn and rewritten_en < rewritten_cn:
        return False

    return True


def _rebuild_paragraph_from_sentence_rewrites(
    text: str,
    ...
    for item in sentence_items:
        ...
        if not original:
            continue
        if not rewritten or rewritten == original or not _is_valid_rewritten(original, rewritten):
            rewritten = _direct_rule_rewrite(original, risk_type)
        if rewritten and original in rebuilt:
            rebuilt = rebuilt.replace(original, rewritten, 1)
    return rebuilt


def _char_overlap_ratio(left: str, right: str) -> float:
    left_chars = {ch for ch in left if not ch.isspace()}
    right_chars = {ch for ch in right if not ch.isspace()}
    if not left_chars:
        return 1.0
    return len(left_chars & right_chars) / max(len(left_chars), 1)


def _looks_like_local_rewrite(original: str, candidate: str) -> bool:
    original_clean = re.sub(r"\s+", "", original)
    candidate_clean = re.sub(r"\s+", "", candidate)
    if not original_clean or not candidate_clean:
        return False

    length_ratio = len(candidate_clean) / max(len(original_clean), 1)
    if length_ratio < 0.7 or length_ratio > 1.25:
        return False

    original_sentences = len(_split_sentences(original_clean))
    candidate_sentences = len(_split_sentences(candidate_clean))
    if candidate_sentences > original_sentences + 1:
        return False

    overlap_ratio = _char_overlap_ratio(original_clean, candidate_clean)
    ...
    return not candidate_clean.startswith(banned_prefixes)
...
def _simple_rewrite_sentence(sentence: str, risk_type: str) -> str:
    ...
    if risk_type in ("aigc", "mixed"):
        ...
        rewritten = re.sub(
            ...
            "基于上述内容，可以看出该部分具有一定的实际意义",
            rewritten,
        )
        # 英文常见模板句
        rewritten = re.sub(
            r"\bWith the development of\b",
            "As",
            rewritten,
            flags=re.IGNORECASE,
        )
        rewritten = re.sub(
            r"\bplays an increasingly important role\b",
            "has been adopted in practical workflows",
            rewritten,
            flags=re.IGNORECASE,
        )
        rewritten = re.sub(
            r"\bis of great significance\b",
            "offers direct practical value",
            rewritten,
            flags=re.IGNORECASE,
        )
    if risk_type in ("duplication", "mixed"):
        ...
        # 直接引用 → 转述（去掉引号，用自己的话表达）
        if re.search(r"[\u201c\"'].{10,80}[\u201d\"']", rewritten):
            rewritten = re.sub(
                r"[\u201c\"'](.{10,80})[\u201d\"']",
                lambda m: f"相关研究认为{m.group(1)}，这一观点在本实验条件下表现为",
                rewritten,
            )
        # 英文被动语态简单转换
        rewritten = re.sub(
            r"\b(\w+)\s+is\s+(completed|implemented|discovered|analyzed|processed|verified|proved|proposed|adopted|applied)\b",
            lambda m: f"researchers {m.group(2)} {m.group(1)}",
            rewritten,
            flags=re.IGNORECASE,
        )
    return rewritten
...
def _fallback_rewrite(
    ...
    # 如果没有匹配到任何规则，对最长的一句做规则改写；如果改写后仍是原文，则不加入 matched_sentences
    if not matched_sentences and sentences:
        longest = max(sentences, key=len)
        rewritten = _direct_rule_rewrite(longest, risk_type)
        if rewritten != longest:
            matched_sentences.append(
                {
                    "original": longest,
                    "risk": "medium",
                    "rewritten": rewritten,
                    "explanation": "系统已按换句子骨架、补具体作用、弱化套话的方式给出可直接替换版本。",
                }
            )

    # 生成改后段落参考：只对真正改写的句子做替换
    ...
    if cnki_aigc_percent is not None and cnki_aigc_percent >= 30:
        diagnosis += (
            f" 知网 AIGC 实测率为 {cnki_aigc_percent:.1f}%，建议按原句做逐字微调，避免扩写。"
        )
    if cnki_dup_percent is not None and cnki_dup_percent >= 20:
        diagnosis += (
    ...
    # 整体建议
    if risk_type == "aigc":
        overall_advice = "离线改写策略：只做逐字微改，优先删套话、换连接词、调整语序，保留原句事实和专有名词。"
    elif risk_type == "duplication":
    ...
    rewritten_paragraph = apply_learned_rewrite_style(rewritten_paragraph)
...
def _direct_rule_rewrite(sentence: str, risk_type: str) -> str:
    ...
    If no rule matches, return the original sentence unchanged — never append advice text."""
    rewritten = _simple_rewrite_sentence(sentence, risk_type)
    if rewritten != sentence and "【请" not in rewritten:
        candidate = apply_learned_rewrite_style(rewritten)
        if _is_valid_rewritten(sentence, candidate):
            return candidate
    ...
    try:
        ...
        if candidate and candidate != sentence and "【请" not in candidate and _is_valid_rewritten(sentence, candidate):
            return candidate
    except Exception:
        pass

    cleaned = apply_learned_rewrite_style(sentence)
    if cleaned != sentence and _is_valid_rewritten(sentence, cleaned):
        return cleaned

    # 根据原文语言选择 fallback 策略；如果无法有效改写，直接返回原文
    cn_chars = sum(1 for ch in sentence if "\u4e00" <= ch <= "\u9fff")
    en_chars = sum(1 for ch in sentence if ch.isascii() and ch.isalpha())

    if cn_chars > en_chars:
        # 中文：只做简单句式微调，不改内容骨架
        if sentence.startswith("通过") and "笔者" not in sentence and "本研究" not in sentence:
            return "笔者" + sentence
        if re.search(r"随着[^，。；]+的发展", sentence):
            return re.sub(
                r"随着([^，。；]+)的发展",
                lambda m: f"伴随{m.group(1)}的推进",
                sentence,
            )
        return sentence
    ...
    # 英文：只做轻微词组替换，不追加新事实
    rewritten = sentence
    rewritten = re.sub(r"\bdesign and implementation\b", "development and implementation", rewritten, flags=re.IGNORECASE)
    rewritten = re.sub(r"\bbased on\b", "built on", rewritten, flags=re.IGNORECASE)
