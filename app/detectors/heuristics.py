...
ACADEMIC_TEMPLATES = (
    # 经典模板句式
    "随着", "不断发展", "日益", "具有重要意义", "本文首先", "其次", "最后",
    "综上所述", "研究表明", "相关研究", "理论基础", "应用路径", "优化策略",
    "存在问题", "提出建议", "进一步推动", "促进发展", "具有一定的参考价值",
    "在此基础上", "从而实现", "有效提升", "显著提高", "综合分析",
    # ChatGPT/通义千问 典型中文输出模式
    "值得注意的是", "需要指出的是", "不可忽视", "不容忽视",
    "总而言之", "概括而言", "简言之", "换言之",
    "这一现象", "这一趋势", "这一问题", "这一领域",
    "为此", "鉴于此", "有鉴于此", "由此可见",
    "不仅如此", "更为重要的是", "尤为关键的是",
    "提供了新的思路", "提供了有益参考", "具有重要的理论价值和实践意义",
    "为…提供了", "为…奠定了基础",
    "在很大程度上", "从根本上", "从本质上看",
    "呈现出", "体现在以下几个方面",
    "对…产生了深远影响", "发挥着重要作用",
...
CONNECTORS = (
    "首先", "其次", "再次", "最后", "同时", "此外", "因此", "然而", "总之",
    "综上", "一方面", "另一方面", "基于此", "与此同时", "进一步", "具体而言",
    # 扩充
    "不仅…而且", "不仅如此", "除此之外", "更为重要的是",
    "归纳起来", "总体来看", "从整体上看", "由此可知", "据此",
    "相应地", "反之", "尽管如此", "事实上",
)

VAGUE_TERMS = (
    "相关", "一定", "有效", "显著", "合理", "科学", "完善", "优化", "提升",
    "推动", "促进", "保障", "机制", "体系", "路径", "策略", "问题", "价值",
    # 扩充：AI 生成高频模糊词
    "探索", "构建", "赋能", "助力", "深化", "拓展", "创新",
    "系统性", "全面性", "针对性", "可行性", "前瞻性",
    "新模式", "新路径", "新格局", "新业态",
)
...
class LexicalDiversityDetector(Detector):
    ...
    def score(self, segment: str, all_segments: list[str]) -> DetectorResult:
        ts = tokens(segment)
        if len(ts) < 40:
            return DetectorResult(self.name, 0.35, self.weight, ["文本片段较短，词汇多样性判断置信度较低"])
...
class SentenceUniformityDetector(Detector):
    ...
    def score(self, segment: str, all_segments: list[str]) -> DetectorResult:
        ...
        if len(ss) < 3:
            return DetectorResult(self.name, 0.40, self.weight, ["句子数量较少，句式均匀度判断置信度较低"])
